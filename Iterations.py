...
if __name__ == "__main__":
    ...
    for i in range(5):
        eps = 10**(2-i)
        plt.figure('x_err')
        add_line(xs_saprek[i], LINESTYLES[(i+1) % 4], MARKERS[i+1], 'SAP-REK($\epsilon = {}$)'.format(eps), 1000, COLORS[i])
        plt.figure('z_err')
        add_line(zs_saprek[i], LINESTYLES[(i+1) % 4], MARKERS[i+1], '$\epsilon = {}$'.format(eps), 1000, COLORS[i])
        plt.figure('xz_err')
        add_line(zs_saprek[i]+ eps* xs_saprek[i], LINESTYLES[(i+1) % 4], MARKERS[i+1], '$\epsilon = {}$'.format(eps), 1000, COLORS[i])

    plt.figure('x_err')
    add_line(xs_rek, LINESTYLES[0], MARKERS[0], 'REK', 1000, 'g')
    plt.figure('z_err')
    add_line(zs_rek, LINESTYLES[0], MARKERS[0], 'REK', 1000, 'g')
    plt.figure('xz_err')
    add_line(zs_rek, LINESTYLES[0], MARKERS[0], 'REK', 1000, 'g')
    ...
    plt.savefig('./figs/gaussian_REK_xs'+str(m)+'x'+str(n)+'.pdf')


    plt.figure('z_err')
    plt.ylim(10**(-14),1000)
    plt.ylabel('$||\mathbf{z}^k - \mathbf{z}^*||^2$')
    plt.savefig('./figs/gaussian_REK_zs'+str(m)+'x'+str(n)+'.pdf')

        
    plt.figure('xz_err')
    plt.ylim(10**(-14),1000)
    plt.ylabel('$||\mathbf{z}^k - \mathbf{z}^*||^2 + \epsilon||\mathbf{x}^k - \mathbf{x}^*||^2$')
    plt.savefig('./figs/gaussian_REK_converges'+str(m)+'x'+str(n)+'.pdf')
    ...
    for i in range(5):
        eps = 10**(2-i)
        plt.figure('x_err')
        line_objects.append(add_line(xs_saprek[i], LINESTYLES[(i+1) % 4], MARKERS[i+1], '$\epsilon = {}$'.format(eps), 1000, COLORS[i]))
        plt.figure('z_err')
        add_line(zs_saprek[i], LINESTYLES[(i+1) % 4], MARKERS[i+1], '$\epsilon = {}$'.format(eps), 1000, COLORS[i])
        plt.figure('xz_err')
        add_line(zs_saprek[i]+ eps* xs_saprek[i], LINESTYLES[(i+1) % 4], MARKERS[i+1], '$\epsilon = {}$'.format(eps), 1000, COLORS[i])
    ...
    plt.figure('x_err')
    line_objects.append(add_line(xs_rek, LINESTYLES[0], MARKERS[0], 'REK', 1000, 'g'))
    plt.figure('z_err')
    add_line(zs_rek, LINESTYLES[0], MARKERS[0], 'REK', 1000, 'g')
    plt.figure('xz_err')
    add_line(zs_rek, LINESTYLES[0], MARKERS[0], 'REK', 1000, 'g')
    ...
    plt.ylim(10**(-14),100)
    plt.ylabel('$||\mathbf{x}^k - \mathbf{x}^*||^2$')
    plt.savefig('./figs/coherent_REK_xs'+str(m)+'x'+str(n)+'.pdf')

    plt.figure('z_err')
    plt.ylim(10**(-14),1000)
    plt.ylabel('$||\mathbf{z}^k - \mathbf{z}^*||^2$')
    plt.savefig('./figs/coherent_REK_zs'+str(m)+'x'+str(n)+'.pdf')

        
    plt.figure('xz_err')
    plt.ylim(10**(-14),1000)
    plt.ylabel('$||\mathbf{z}^k - \mathbf{z}^*||^2 + \epsilon||\mathbf{x}^k - \mathbf{x}^*||^2$')
    plt.savefig('./figs/coherent_REK_converges'+str(m)+'x'+str(n)+'.pdf')
